...
| **7** | How Will You Get Customers? | R5 → R6 | `3.1` Meta · `3.2` Google · `3.4` TikTok · `3.6` affiliate & influencer · `3.8` creative · `3.9` brand-building | The budget they have been paying by default for five months is finally theirs. Channel CAC and channel churn differ; the cohort ledger remembers where each customer came from. Brand-building is the contrast: it buys no orders this month, decays if you stop, and carries most of the organic traffic that makes every later performance rupee cheaper. |
